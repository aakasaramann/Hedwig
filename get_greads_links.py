...
def time_took(f):
    ...
    def func(*args, **kwargs):
        st = time.time()
        res = f(*args, **kwargs)
        print("time took to run {} is {}".format(f.__name__,
                                                 (time.time() - st)))
        return res
...
def match_score(entry, title, author):
    ...
    if len(s_) < 3:
        s_.extend([""] * (3 - len(s_)))
    return 0.75 * lev.ratio(title.lower(), s_[0].lower()) + 0.25 * lev.ratio(
        author.lower(), s_[2].lower())
...
def get_greads_links(books, top_result=True):
    ...
    """
    for book in books:
        source = urlopen("https://www.goodreads.com/search?q={}".format(
            "+".join(book[0].split())))
        ...
        if top_result:
            ...
            while (match is None) and i < len(tables_[0].contents):
                ...
                if "schema.org/Book" in ((hasattr(entry, "attrs")
                                          and entry.attrs.get("itemtype", ""))
                                         or ""):
                    match = entry
                i += 1
        else:
        ...
        book.append("https://www.goodreads.com" +
                    (title_link_[0].attrs["href"].split("?")[0]))
    return books
