...
def get_all_lists(soup):
    ...
    if soup.find("a", text="More lists with this book..."):
        ...
        while soup.find("a", {"class": "next_page"}) and i <= 10:
            time.sleep(2)
            next_url = ("https://www.goodreads.com" +
                        soup.find("a", {"class": "next_page"})["href"])
            source = urlopen(next_url)
            soup = bs4.BeautifulSoup(source, "lxml")

            lists += [
                node.text for node in soup.find_all("div", {"class": "cell"})
            ]
            i += 1
...
def get_rating_distribution(soup):
    distribution = re.findall(r"renderRatingGraph\([\s]*\[[0-9,\s]+",
                              str(soup))[0]
...
def get_description(soup):
    try:
        return (soup.find(
            "div", class_="readable stacked").find_all("span")[1].text.replace(
                '"', "'"))
    except:
        return ""
...
def get_author_thumbnail(soup):
    try:
        auth_img = soup.find("div", {
            "class": "bookAuthorProfile__photo"
        }).attrs["style"]
...
def get_author_url(soup):
    try:
        auth_name = soup.find("div", {
            "class": "bookAuthorProfile__name"
        }).contents
...
def scrape_book(book_id):
    ...
    return {
        ...
        "book_thumbnail":
        get_book_thumbail(soup),
        "author_thumbnail":
        get_author_thumbnail(soup),
        "author_url":
        get_author_url(soup),
    }
...
def condense_books(books_directory_path):
    ...
    for file_name in os.listdir(books_directory_path):
        if (file_name.endswith(".json") and not file_name.startswith(".")
            ...
            _book = json.load(
                open(books_directory_path + "/" + file_name,
